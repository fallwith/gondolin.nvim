...
function M.get_all_plugins()
	...
	local runtime_files = vim.api.nvim_get_runtime_file("**/gondolin/groups/plugins", false)
...
function M.setup(colors, opts)
	...
	for plugin, enabled in pairs(enabled_plugins) do
		if enabled then
			...
			local ok, res = pcall(require, "gondolin.groups.plugins." .. plugin)
			if not ok then
				vim.notify(
