...
> [!NOTE]
> Gondolin colors floating-window borders (the `FloatBorder`, `NormalFloat` and
> `FloatTitle` groups), but it does not decide _whether_ a border is drawn — that
> is an editor setting, not a color. If hover (`K`), diagnostics or other floats
> appear without a border, enable them yourself. On Neovim 0.11+ the simplest way
> is the global option:
>
> ```lua
> vim.o.winborder = "rounded" -- "single", "double", "solid", ...
> ```
